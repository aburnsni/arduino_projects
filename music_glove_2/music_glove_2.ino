...
int16_t AcX,AcY,AcZ,Tmp,GyX,GyY,GyZ;  // Max Ac value = 32767
int16_t AcXmax, AcYmax, AcZmax;
void setup(){
  Wire.begin();
...
//  GyZ=Wire.read()<<8|Wire.read();  // 0x47 (GYRO_ZOUT_H) & 0x48 (GYRO_ZOUT_L)
  ...
  AcX = map(AcX, 0, 32767, 0,127);
  AcY = map(AcY, 0, 32767, 0,127);
  AcZ = map(AcZ, 0, 32767, 0,127);
  Serial.print(AcX);
